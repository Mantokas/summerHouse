...
import lt.baraksoft.summersystem.portal.view.ReservationPaymentView;
...
public class ReservationPaymentController implements Serializable {
	...
	public String initAndBeginConversation() {
		...
		if (!FacesContext.getCurrentInstance().isPostback() && conversation.isTransient()) {
			...
			reservationPaymentView
					.setReservationsList(reservationPaymentHelper.getReservationsBySummerhouse(reservationPaymentView.getSelectedSummerhouse().getId()));
			buildDateConstraint();
	...
	private void calculateSummerhousePrice() {
		BigDecimal reservationPeriodInDays = BigDecimal
				.valueOf(ChronoUnit.DAYS.between(reservationFrom.toInstant().atZone(ZoneId.systemDefault()).toLocalDate(),
	...
	private void checkReservations(ReservationView reservationView) {
		if (reservationPaymentView.getReservationFrom().isAfter(reservationPaymentView.getReservationTo())
				|| (reservationPaymentView.getReservationFrom().isBefore(reservationView.getDateTo())
						&& reservationPaymentView.getReservationTo().isAfter(reservationView.getDateFrom()))) {
			dateIncorrect = true;
		}
	}
	...
	public void goToThirdStep() {
		...
		if (!selectedServiceViews.isEmpty()) {
			...
			selectedServices.stream().forEach(service -> reservationPaymentView.setServicesReservationPrice(
					reservationPaymentView.getServicesReservationPrice().add(service.getPrice().multiply(reservationPeriodInWeeks))));
		}
	...
	private void createReservation() {
		...
		reservationPaymentView.setPointsAfter(reservationPaymentView.getPointsBefore() - reservationPaymentView.getServicesReservationPrice().intValue()
